...
\item CIRCL working on: \texttt{p434, p503 and p751}
	...
	\end{itemize}
\end{itemize}
Furthermore, the benchmarking suite provides classical elliptic curve Diffie-Hellman (ECDH) based on \gls{openssl}. Since \gls{SIDH} is a candidate to replace current Diffie-Hellman algorithms, \gls{ECDH} is intended as reference value: The objective is to compare optimized modern \gls{ECDH} with quantum-secure \gls{SIDH}.\\
...
\begin{figure}[H]
  ...
  {Flow chart of the benchmarking suite. For each \gls{SIDH} implementation, the source code is compiled and benchmarked multiple times. The benchmarking results are visualized in different format: Graphs, \gls{HTML} and Latex.
  } \label{fig:app_flow}
\end{figure}
...
To add a new implementation to the benchmarking suite it is recommended to consider already existing implementations. Adding a \gls{SIDH} implementation to the benchmarking suite requires the following steps:
\begin{enumerate}
\itemsep0em 

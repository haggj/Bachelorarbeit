if [ $1 = "build" ]; then
    ...
    sudo docker run --privileged -t -v $PWD/.tmp:/usr/src/app/data benchmark
    # Extract results
    sudo rm -rf data/
    cp -r .tmp/ data/
    sudo rm -r .tmp/
    ...
    sudo docker run benchmark pytest --color=yes -o log_cli=true --log-cli-level=INFO src/test/
